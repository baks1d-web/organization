...
# ---------------- Habits API ----------------
def _habit_range(view: str, anchor: date) -> tuple[date, date]:
...
def habits_list():
    ...
    anchor_iso = request.args.get("date") or ""
    try:
        anchor = date.fromisoformat(anchor_iso) if anchor_iso else date.today()
    except Exception:
        anchor = date.today()

    start, end = _habit_range(view, anchor)
    days = [start + timedelta(days=i) for i in range((end - start).days + 1)]

    habits = Habit.query.filter_by(user_id=uid, is_active=True).order_by(Habit.id.asc()).all()
    ...
    if habit_ids:
        checks = HabitCheck.query.filter(
            ...
            HabitCheck.day >= start,
            HabitCheck.day <= end,
        ).all()

    by_habit: dict[int, set[str]] = {}
    for c in checks:
        by_habit.setdefault(c.habit_id, set()).add(c.day.isoformat())

    items = []
    overall_done = 0
    overall_total = len(days) * len(habits)

    for h in habits:
        done_days = by_habit.get(h.id, set())
        overall_done += len(done_days)
        items.append({
            "id": h.id,
            "title": h.title,
            "done": len(done_days),
            "total": len(days),
            "checks": {d: True for d in done_days},
        })

    day_payload = [{"date": d.isoformat(), "label": ["ПН","ВТ","СР","ЧТ","ПТ","СБ","ВС"][d.weekday()], "day": d.day} for d in days]
...
def habits_create():
    ...
    h = Habit(user_id=uid, title=title, is_active=True)
    db.session.add(h)
    db.session.commit()
    return jsonify({"ok": True, "habit": {"id": h.id, "title": h.title}})


@api_bp.post("/habits/<int:habit_id>/check")
@jwt_required()
def habits_check(habit_id: int):
    uid = get_jwt_identity()
    h = Habit.query.filter_by(id=habit_id, user_id=uid).first()
    if not h:
    ...
    existing = HabitCheck.query.filter_by(habit_id=h.id, day=d).first()

    # If done is not provided -> toggle
    if done is None:
        done = existing is None

    if done and existing is None:
        db.session.add(HabitCheck(habit_id=h.id, day=d))
    if (not done) and existing is not None:
        db.session.delete(existing)

    db.session.commit()
    return jsonify({"ok": True, "habit_id": h.id, "date": d.isoformat(), "done": bool(done)})


@api_bp.delete("/habits/<int:habit_id>")
@jwt_required()
